...
{
    public Value Eval(Node node, ValueEnvironment environment)
    {
        switch (node)
        {
    ...
    private Value EvalHashLiteral(HashLiteral hashLiteral, ValueEnvironment environment)
    {
    ...
    private static ValueEnvironment ExtendFunctionEnv(
        (IReadOnlyList<Identifier> Parameters, BlockStatement Body, ValueEnvironment Environment) functionData,
        IReadOnlyList<Value> args)
    {
        var env = new ValueEnvironment(functionData.Environment, args.Count);
    ...
    private static Value EvalIdentifier(Identifier identifier, ValueEnvironment environment)
    {
        var (val, ok) = environment.Get(identifier.Value);
        if (ok) return val;
        if (!ValueBuiltins.TryGet(identifier.Value, out var builtinValue))
            return Value.Error($"identifier not found: {identifier.Value}");
        return builtinValue;
    }

    private Value EvalBlockStatement(BlockStatement blockStatement, ValueEnvironment environment)
    {
    ...
    private Value EvalProgram(ProgramNode programNode, ValueEnvironment environment)
    {
        var result = Value.Null();
    ...
    private Value EvalIfExpression(IfExpression ifExpression, ValueEnvironment environment)
    {
