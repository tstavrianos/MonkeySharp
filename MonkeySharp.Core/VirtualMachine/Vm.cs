...
namespace MonkeySharp.Core.VirtualMachine
{
    public class Vm
    {
        ...
        public Vm(ByteCode bytecode)
        {
            ...
            _frames.Push(new Frame(closure, 0));
        ...
        public string Run()
        {
            while (CurrentFrame().Ip < CurrentFrame().Instructions().Length - 1)
            {
                ...
                switch (op)
                {
                    ...
                    {
                        var constIndex = BinaryPrimitives.ReadUInt16BigEndian(ins.AsSpan(ip + 1));
                        CurrentFrame().Ip += 2;
                    ...
                    case OpCode.Jump:
                    {
                        var pos = BinaryPrimitives.ReadUInt16BigEndian(ins.AsSpan(ip + 1));
                    ...
                    {
                        var globalIndex = BinaryPrimitives.ReadUInt16BigEndian(ins.AsSpan(ip + 1));
                        CurrentFrame().Ip += 2;
                    ...
                    case OpCode.Array:
                    {
                        var numElements = BinaryPrimitives.ReadUInt16BigEndian(ins.AsSpan(ip + 1));
                        CurrentFrame().Ip += 2;
                    ...
                    case OpCode.Closure:
                    {
                        var constantIndex = BinaryPrimitives.ReadUInt16BigEndian(ins.AsSpan(ip + 1));
        ...
        {
            var hashedPairs = new Dictionary<HashKey, (IHashableObject Key, IObject Value)>();
        ...
        private string ExecuteBinaryOperation(OpCode op)
        {
            ...
            if (left is StringObject leftStr && right is StringObject rightStr)
                return ExecuteBinaryStringOperation(leftStr, op, rightStr);

            return $"unsupported types for binary operation: {left.GetType().Name} {right.GetType().Name}";
        }

        private string ExecuteBinaryStringOperation(StringObject leftStr, OpCode op, StringObject rightStr)
        {
            if (OpCode.Add == op)
